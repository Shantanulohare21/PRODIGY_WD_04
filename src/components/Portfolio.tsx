    ...
    >
      ...
      >
        ...
        <div className="w-full md:w-[55%] p-10 md:p-14 flex flex-col bg-[#111827] text-slate-100">
          <div className="mb-8">
            <span className="text-teal-400 font-bold text-xs uppercase tracking-widest mb-3 block">{t.portfolio.specialty}</span>
            <h3 className="text-3xl md:text-4xl font-extrabold text-white mb-6 leading-tight">{project.title}</h3>
            <div className="flex flex-wrap gap-2.5 mb-8">
              {(project.tech || []).map((tech: string) => (
                <span key={tech} className="px-4 py-1.5 bg-teal-500/10 text-teal-300 text-[10px] font-bold rounded-full border border-teal-500/20 uppercase tracking-widest shadow-sm">{tech}</span>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-10 mb-14">
            <div className="flex gap-6 group">
              ...
              <div className="flex-1">
                <h4 className="text-sm font-bold text-white uppercase tracking-widest mb-2">{t.portfolio.challenge}</h4>
            ...
            <div className="flex gap-6 group">
              ...
              <div className="flex-1">
                <h4 className="text-sm font-bold text-white uppercase tracking-widest mb-2">{t.portfolio.solution}</h4>
            ...
            <div className="flex gap-6 group">
              ...
              <div className="flex-1">
                <h4 className="text-sm font-bold text-white uppercase tracking-widest mb-2">{t.portfolio.impact}</h4>
          ...
          <div className="mt-auto flex gap-6 pt-10 border-t border-white/5">
            <a href={project.live || '#'} target="_blank" rel="noopener noreferrer" className="flex-1 px-8 py-4 bg-teal-500 text-white rounded-2xl font-bold text-xs text-center border border-teal-400/50 hover:bg-teal-600 transition-all flex items-center justify-center gap-3 shadow-lg active:scale-95 group">
              {t.portfolio.exploreBtn} <ExternalLink className="w-4 h-4 transition-transform group-hover:translate-x-1 group-hover:-translate-y-1" />
            </a>
            <a href={project.repo || '#'} target="_blank" rel="noopener noreferrer" className="flex-1 px-8 py-4 bg-white/5 text-white rounded-2xl font-bold text-xs text-center hover:bg-white/10 transition-all border border-white/10 flex items-center justify-center gap-3 shadow-xl active:scale-95 group">
              {t.portfolio.sourceBtn} <Github className="w-4 h-4" />
            </a>
          </div>
        </div>
      </motion.div>
    ...
    >
      <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-20">
          <div className="flex flex-col gap-6">
            <div className="flex items-center gap-3">
              ...
              <span className="text-teal-400 font-bold text-sm tracking-widest uppercase">{t.portfolio.badge}</span>
            </div>
            <h2 className="text-4xl md:text-5xl font-extrabold text-white leading-tight tracking-tight reveal-text">{t.portfolio.title}</h2>
          </div>
              ...
              <div className="px-1">
                <div className="flex flex-wrap gap-2 mb-4">
                  {project.tech.slice(0, 3).map((tech, idx) => (
                    <span key={idx} className="text-[10px] font-bold uppercase text-teal-400/80 tracking-widest">{tech}</span>
                  ))}
                </div>
        ...
        <div className="flex justify-center">
          <motion.a
            href="https://github.com/Shantanulohare21"
            target="_blank"
          ...
          >
            {t.portfolio.repoBtn} <Github className="w-5 h-5" />
          </motion.a>
        </div>
      </div>
